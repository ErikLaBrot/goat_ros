"""Launch the GOAT field robot app.

This entrypoint is robot-side only. Remote joystick input is expected to run on
an operator workstation and publish scaled commands to the shared cmd/vesc
topic.
"""
...
def generate_launch_description():
    """Build the launch description for field robot bringup."""
    ...
    robot_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                launch_share_dir,
                "launch",
                "robot_d435_visual_slam.launch.py",
            )
        ),
        launch_arguments={
