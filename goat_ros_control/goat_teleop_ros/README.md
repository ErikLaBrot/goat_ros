...
`goat_teleop` is a ROS 2 Humble joystick teleop package for GOAT remote
operator control. It consumes `sensor_msgs/msg/Joy` samples on `/joy` and
publishes `goat_vesc_ros/msg/VescControlCommand` on the canonical `cmd/vesc`
topic for manual vehicle control.
...
- `command_scale` (`double`, default: `0.30`): Maximum absolute duty command sent to `goat_vesc_ros` from the configured throttle axis.
...
- `servo_amplitude` (`double`, default: `0.5`): Steering excursion added around `servo_center` before clamping to `[0.0, 1.0]`.
- `deadband` (`double`, default: `0.03`): Normalized throttle and steering input inside `[-deadband, deadband]` is treated as zero.
- `steer_tau_s` (`double`, default: `0.08`): Steering first-order low-pass filter time constant in seconds. Use `0.0` to disable steering filtering.
- `throttle_tau_s` (`double`, default: `0.12`): Throttle first-order low-pass filter time constant in seconds. Use `0.0` to disable throttle filtering.
- `max_steer_rate` (`double`, default: `5.0`): Maximum conditioned steering change in normalized joystick units per second. Use `0.0` to disable steering slew limiting.
- `max_throttle_rate` (`double`, default: `3.5`): Maximum conditioned throttle change in normalized joystick units per second. Use `0.0` to disable throttle slew limiting.
...
remote operator driving, where small stick noise should disappear and quick
stick steps should soften without making the vehicle feel delayed.

## Launch Entry Points

- `teleop.launch.py`: Starts `joy_node` plus `goat_joy` for a remote operator
  machine that shares the robot's ROS graph. Accepts `config_file`, `joy_dev`,
  and `deadzone`.

## Config Profiles

- `goat_joy.yaml`: Remote field-control mapping for publishing `cmd/vesc`.
...
- Config files: `config/goat_joy.yaml` provides the controller mapping, invert flags, throttle scale, steering scale, and conditioning configuration
...
Start teleop on a remote operator client:

```bash
ros2 launch goat_teleop teleop.launch.py
```

Override the joystick device or deadzone at launch time when needed:

```bash
ros2 launch goat_teleop teleop.launch.py joy_dev:=/dev/input/js1 deadzone:=0.02
```
...
- The package keeps one launch file and one config profile for the remote
  operator role.
