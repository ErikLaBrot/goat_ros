...
: Node("goat_joy", options)
{
  ...
  command_topic_ = this->declare_parameter<std::string>("command_topic", "cmd/vesc");
  ...
  command_publisher_ = this->create_publisher<std_msgs::msg::Float32>(command_topic_, 10);
...
void GoatJoy::joyCallback(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  ...
  if (isButtonPressed(*msg, enable_button_)) {
    const double throttle = getAxisValue(*msg, throttle_axis_, invert_throttle_);
    command = std::clamp(throttle * command_scale_, -command_scale_, command_scale_);
  }
...
void GoatJoy::publishCommand(double command)
{
  ...
  message.data = static_cast<float>(command);
